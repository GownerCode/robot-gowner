...
module.exports = {
    ...
    async execute(interaction) {
        ...
        let replyString = ``
        replyString +=
            `The next ${mention ? `<@&${roles[interaction.guild.id].movie_night_role_id}>` : 'Movie Night'} will be on\n**<t:${next}:f> (<t:${next}:R>)**\n`;
        if (nextMovie != null) {
            replyString +=
                `We will be watching\n***${nextMovie.title} (${nextMovie.year})***\n`
        } else {
            replyString +=
                `The next movie has not been determined yet. Keep an eye out for messages in <#${channels[interaction.guild.id].movie_info_channel}>!\n`
        }
        replyString +=
